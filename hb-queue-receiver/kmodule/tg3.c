	...
	while (sw_idx != hw_idx && budget > 0) {
		...
		if (desc->type_flags & RXD_FLAG_VLAN &&
			...
			__vlan_hwaccel_put_tag(skb, htons(ETH_P_8021Q),
                ...
                // msx
                skb->napi_id = tnapi->irq_vec;
                //printk("[msx] tnapi->irq_vec = %u\n", tnapi->irq_vec);
...
static void tg3_periodic_fetch_stats(struct tg3 *tp)
{
	...
	TG3_STAT_ADD32(&sp->rx_undersize_packets, MAC_RX_STATS_UNDERSIZE);
        ...
        // msx
	//TG3_STAT_ADD32(&sp->rxbds_empty, RCVLPC_DROP_FILTER_CNT);
...
// msx
//static const int my_tp_regs_max = 5;
void  *my_tp_regs[5];
EXPORT_SYMBOL(my_tp_regs);

int my_tp_regs_count = 0;
EXPORT_SYMBOL(my_tp_regs_count);
				  ...
				  const struct pci_device_id *ent)
{
	...
	pci_save_state(pdev);
        ...
        // msx
        printk("[msx] The address of tp->regs for net_device %s = %p\n", tp->dev->name, tp->regs);
        if(my_tp_regs_count < 5) {
                my_tp_regs[my_tp_regs_count] = tp->regs;
                my_tp_regs_count ++;
        }
