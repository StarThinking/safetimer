...
extern "C" long st_send_hb(int);
extern "C" void st_update(void);
extern "C" long st_get_valid(int node);
...
{
  dout(10) << "[msx] heartbeat_check" << dendl;
  ...
  for (map<int,HeartbeatInfo>::iterator p = heartbeat_peers.begin();
    ...
    dout(25) << "heartbeat_check osd." << p->first
	     ...
	     << dendl;
      long valid_t = st_get_valid(p->first);
    if (p->second.is_unhealthy(cutoff) && valid_t < cutoff) {
      dout(10) << "[msx] osd " << p->first << " is_unhealthy" << dendl;
...
long st_get_valid(int node) {
    struct sockaddr_in request_server;
    int fd;
    unsigned int len;
    int count;
    long request[2];
    ...
    memset(&request_server, '0', sizeof(request_server));
    request_server.sin_addr.s_addr = inet_addr("127.0.0.1");
    request_server.sin_family = AF_INET;
    request_server.sin_port = htons(5678);
    fd = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP);

    request[0] = 1;
    request[1] = node;

    count = sendto(fd, &request, sizeof(long)*2, 0, (struct sockaddr *) &request_server, sizeof(request_server));
    if (count != sizeof(long)*2)
        perror("request sendto");

    if(recvfrom(fd, &valid_time, sizeof(long), 0, (struct sockaddr *) &request_server, &len) != sizeof(long))
        perror("recvfrom");

    printf("valid_time = %ld\n", valid_time);
    close(fd);
    return valid_time;
}

long st_send_hb(int node) {
    struct sockaddr_in request_server;
    int fd;
    unsigned int len;
    int count;
    ...
    request_server.sin_addr.s_addr = inet_addr("127.0.0.1");
    request_server.sin_family = AF_INET;
    request_server.sin_port = htons(5003);
    fd = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP);

    clock_gettime(CLOCK_MONOTONIC, &spec);
    now_t = spec.tv_sec * 1000 + spec.tv_nsec/1.0e6;
    request[0] = node;
    request[1] = now_t + 1000;

    count = sendto(fd, &request, sizeof(long)*2, 0, (struct sockaddr *) &request_server, sizeof(request_server));
    if (count != sizeof(long)*2)
        perror("request sendto");

    if(recvfrom(fd, &reply, sizeof(long), 0, (struct sockaddr *) &request_server, &len) != sizeof(long))
        perror("recvfrom");
...
{
  dout(10) << "[msx] send_failures" << dendl;
