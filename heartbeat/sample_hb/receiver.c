...
static int st_check_timeout(long app_id, char *node_ip, long epoch_start) {
        long st_request[3];
        int st_ret = 0;

        memset(st_request, 0, sizeof(long)*3);
        st_request[0] = app_id;
        st_request[1] = htonl(inet_addr(node_ip));
        st_request[2] = epoch_start;

        if(sendto(st_client_fd, st_request, MSGSIZE*3, 0, (struct sockaddr *) &st_server,
                sizeof(st_server)) != MSGSIZE*3) {
                perror("send st_request");
        }
        return st_ret;
}
...
static void expiration_check_for_epoch(long epoch) {
	...
	/* Before perfroming expiration check, grab the lock. */
        ...
        if (ip_list != NULL) {
               ...
               while (next != NULL) {
                        int st_ret = st_check_timeout(APP_ID, (char*) next->val, epoch_to_time(epoch)-timeout_interval);
                        if (st_ret == 1) {
                                ...
                                long new_epoch = epoch + 1;
                                char saddr[INET_ADDRSTRLEN];
                                list_t **_ip_list;

                                memcpy(saddr, (char*) next->val, INET_ADDRSTRLEN);
                                
                                printf("st_ret is %d, no timeout actually.\n", st_ret);
                	        
                                //  add ip_node into the next epoch list
                                _ip_list = (list_t**) ht_get(&epoch_list_ht, &new_epoch, sizeof(long), &value_size);
                	        if (_ip_list == NULL) {
                        	        list_t *tmp = list_new();
                        	        tmp->match = ip_equal;
                        	        tmp->free = free_val;
                       	 	        _ip_list = &tmp;
                        	        ht_insert(&epoch_list_ht, &new_epoch, sizeof(long), _ip_list, sizeof(list_t*));
                	        }

                	        if (list_find(*_ip_list, saddr) == NULL) {
                        	        // Allocate the memory of _saddr in heap
                        	        char *_saddr = (char*) calloc(INET_ADDRSTRLEN, sizeof(char));
                        	        strcpy(_saddr, saddr);
                        	        list_rpush(*_ip_list, list_node_new(_saddr));
                        	        printf("NHeartbeat server: add node %s into the node list for epoch %ld.\n", 
                                                saddr, epoch);
                	        }
                        }
               ...
               ht_remove(&epoch_list_ht, &epoch, sizeof(long));
                ...
                printf("Checker: ip list is empty.\n");
        }
